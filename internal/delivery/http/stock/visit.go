...
func (h *Handler) GetVisitsByID(w http.ResponseWriter, r *http.Request) {
	var (
		result   interface{}
		metadata interface{}
		err      error
...
func (h *Handler) CreateVisit(w http.ResponseWriter, r *http.Request) {
	...
	err = h.stockSvc.CreateVisit(ctx, Visit)
	if err != nil {
		resp = httpHelper.ParseErrorCode(err.Error())
		log.Printf("[ERROR][%s][%s] %s | Reason: %s", r.RemoteAddr, r.Method, r.URL, err.Error())
		return
...
func (h *Handler) UpdateVisit(w http.ResponseWriter, r *http.Request) {
	...
	err = h.stockSvc.UpdateVisit(ctx, visit)
	if err != nil {
		resp = httpHelper.ParseErrorCode(err.Error())
		log.Printf("[ERROR][%s][%s] %s | Reason: %s", r.RemoteAddr, r.Method, r.URL, err.Error())
		return
...
func (h *Handler) DeleteVisit(w http.ResponseWriter, r *http.Request) {
	...
	err := h.stockSvc.DeleteVisit(ctx, id)
	if err != nil {
		resp = httpHelper.ParseErrorCode(err.Error())
		log.Printf("[ERROR][%s][%s] %s | Reason: %s", r.RemoteAddr, r.Method, r.URL, err.Error())
		return
